#!/usr/bin/env bash
# =============================================================================
# triage.sh — Nine-Tailed Fox CDT Competition
# Ubuntu 24.04 monitoring & auditing setup
...
PACKAGES=(
    ...
    fail2ban         # brute-force mitigation via log parsing
    psad             # port scan detection (iptables log based)
    net-tools        # netstat
    lsof             # open file / socket listing
    sysstat          # sar, iostat, pidstat
    acct             # process accounting (lastcomm, sa)
    chkrootkit       # second rootkit scanner
    inotify-tools    # inotifywait for real-time file watch
    curl wget        # needed by some detections / clones
    clang llvm       # needed if building XDP firewall
    libbpf-dev       # XDP dependency
    linux-headers-"$(uname -r)"  # XDP / module build dep
    bpftool          # inspect loaded BPF programs (remove later if desired)
)

apt-get install -y "${PACKAGES[@]}" 2>&1 | tee -a "$LOG" || warn "Some packages failed — check log"
...
if [[ -f "$SCRIPT_DIR/audit.rules" ]]; then
    ...
    warn "audit.rules not found next to triage.sh — skipping rule deploy"
fi
...
systemctl enable acct 2>/dev/null && systemctl start acct 2>/dev/null || true
...
# Write a focused config to reduce noise on competition boxes
cat > /etc/aide/aide.conf.d/99-competition.conf <<'EOF'
...
aide --init --config /etc/aide/aide.conf 2>&1 | tee -a "$LOG" &
AIDE_PID=$!
log "AIDE database init running in background (PID $AIDE_PID)"
log "After it completes, promote with: mv /var/lib/aide/aide.db.new /var/lib/aide/aide.db"
...
grep -rn "LD_PRELOAD" /etc/environment /etc/profile /etc/profile.d/ \
    /etc/bash.bashrc /root/.bashrc /root/.profile 2>/dev/null | tee -a "$LOG" \
...
# =============================================================================
log "=== Pinning firewall packages to prevent apt reinstall ==="

cat > /etc/apt/preferences.d/no-firewall <<'EOF'
Package: iptables
Pin: release *
Pin-Priority: -1
...
ok "iptables / nftables / ufw / firewalld pinned to -1"
